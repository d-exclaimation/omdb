...
const FilmPreview: FC<FilmPreviewProps> = ({
  ...
  return (
    <Link
      key={filmId}
      to={`/film?id=${filmId}`}
      className="group flex h-full w-64 flex-shrink-0 flex-col overflow-hidden rounded bg-white dark:bg-zinc-900"
    >
      <div className="aspect-[4/3] w-64 overflow-hidden rounded object-cover dark:bg-zinc-900">
        <Img
          className="aspect-[4/3] w-64 rounded object-cover transition-all group-hover:scale-110"
      ...
      <div className="flex w-full flex-col gap-1 py-2">
        <h3 className="max-w-[90%] truncate text-sm font-semibold group-hover:text-zinc-500 group-hover:underline dark:text-white">
          {title}
        </h3>
